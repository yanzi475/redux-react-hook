...
>({defaultAreEqual = referenceEqual}: {defaultAreEqual?: (a: unknown, b: unknown) => boolean} = {}): {
  StoreContext: React.Context<TStore | null>;
  useMappedState: <TResult>(mapState: (state: TState) => TResult, areEqual?: (a: TResult, b: TResult) => boolean) => TResult;
  ...
  function useMappedState<TResult>(
    ...
    areEqual: (a: TResult, b: TResult) => boolean = defaultAreEqual,
  ): TResult {
    ...
    useIsomorphicLayoutEffect(() => {
      ...
      const checkForUpdates = () => {
        ...
        if (!areEqual(newDerivedState, lastStateRef.current)) {
